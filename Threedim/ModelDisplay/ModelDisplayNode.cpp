...
void main()
{
  ...
  vec3 e = normalize( vec3( mat.matrixModelView * p ) );
  vec3 n = normalize( mat.matrixNormal * normal );
...
void main ()
{
  //vec2 uv = asin(v_normal.xy) / 3.1415926 + 0.5;
  fragColor = texture(y_tex, v_normal.xy);
}
)_";
...
private:
  ...
  void initPasses(RenderList& renderer, const Mesh& mesh)
  {
    ...
    for(auto& attr : mesh.vertexAttributes)
    {
  ...
  void init(RenderList& renderer) override
  {
    ...
    m_inputTarget = score::gfx::createRenderTarget(renderer.state, QRhiTexture::RGBA8, renderer.state.renderSize);
    ...
    std::tie(texCoordVS, texCoordFS) = score::gfx::makeShaders(renderer.state, model_display_vertex_shader, model_display_fragment_shader);
    std::tie(triplanarVS, triplanarFS) = score::gfx::makeShaders(renderer.state, model_display_vertex_shader_triplanar, model_display_fragment_shader_triplanar);
    std::tie(sphericalVS, sphericalFS) = score::gfx::makeShaders(renderer.state, model_display_vertex_shader_spherical, model_display_fragment_shader_spherical);
    std::tie(viewspaceVS, viewspaceFS) = score::gfx::makeShaders(renderer.state, model_display_vertex_shader_viewspace, model_display_fragment_shader_viewspace);
    std::tie(barycentricVS, barycentricFS) = score::gfx::makeShaders(renderer.state, model_display_vertex_shader_barycentric, model_display_fragment_shader_barycentric);
  ...
  update(RenderList& renderer, QRhiResourceUpdateBatch& res) override
  {
    ...
    model.rotate(QQuaternion::fromEulerAngles(n.rotation[0], n.rotation[1], n.rotation[2]));
    ...
    projection.perspective(90, qreal(renderer.state.renderSize.width()) / renderer.state.renderSize.height(), 0.0001, 1000.);
    QMatrix4x4 view;
...
void ModelDisplayNode::process(Message&& msg)
{
  ...
  for (const gfx_input& m: msg.input)
  {
    ...
    else if(auto val = ossia::get_if<ossia::geometry>(&m))
    {
